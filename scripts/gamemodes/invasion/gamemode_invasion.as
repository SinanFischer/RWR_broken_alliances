...
#include "water_pistol.as"
#include "vip_manager.as"
#include "color_player.as"
#include "halloween3.as"
#include "halloween_enemy_skill.as"
#include "kill_commander.as"
#include "self_heal.as"
#include "halloween_miniboss_manager.as"
#include "dancing_grenade.as"
#include "snowman.as"
...
class GameModeInvasion : GameMode, UnlockRemoveListener, UnlockListener {
	...
	/*	addTracker(IceTrade(this)); */
		...
		addTracker(Jax(this));
		addTracker(Halloween3(this));
		addTracker(SantaStab(this));
		addTracker(EasterGrenade(this));
		addTracker(EasterTriggerTracker(this));
		addTracker(ColorPlayer(this));
	...
	protected void setupExperimentalFeatures() {
		...
		addTracker(VipManager(this));		// normal: VipManager(this), Variant for "xmas santa": VipManager(this, "xmas santa")
		addTracker(WaterPistol(this));
		addTracker(HalloweenEnemySkill(this));
		addTracker(HalloweenMinibossManager(this));
		addTracker(EnemyCommanderEvent(this));	// for Halloween addTracker(EnemyCommanderEvent(this, "evil commander"));
		addTracker(SelfHeal(this));
		//addTracker(SnowManager(this));	// enabled during xmas event
	...
	protected void setupDogs() {
		// Hunde für alle Fraktionen (Spieler + Feinde), spawn_score 0.04 (~4%)
		for (uint i = 0; i < m_factions.size(); ++i) {
			...
			command.setFloatAttribute("spawn_score", 0.04f);
			getComms().send(command);
		}
	}

	// --------------------------------------------
    protected void setupBunny() {
        _log("Setting up bunny faction...");
        {
            // disable bunnies in friendly faction
            XmlElement command("command");
            command.setStringAttribute("class", "faction");
            command.setIntAttribute("faction_id", 0);
            command.setStringAttribute("soldier_group_name", "easter bunny");
            command.setFloatAttribute("spawn_score", 0.0f);
            getComms().send(command);
        }
            ...
            const FactionConfig@ config = m_factions[i].m_config;
        {
            ...
            command.setIntAttribute("faction_id", i);
            command.setStringAttribute("soldier_group_name", "easter bunny");
            command.setFloatAttribute("spawn_score", 0.0f);		// 0.005 during Easter event
            getComms().send(command);
        }
	...
	protected void setupRipper() {
			...
			const FactionConfig@ config = m_factions[i].m_config;
		{
			...
			command.setIntAttribute("faction_id", i);
			command.setStringAttribute("soldier_group_name", "elite ripper");
			command.setFloatAttribute("spawn_score", 0.0f);		// 0.005 (was 0.008) during Halloween event
			getComms().send(command);
		}
	...
	protected void setupDemon() {
		{
			// disable Demons in friendly faction
			XmlElement command("command");
			command.setStringAttribute("class", "faction");
			command.setIntAttribute("faction_id", 0);
			command.setStringAttribute("soldier_group_name", "demon sniper");
			command.setFloatAttribute("spawn_score", 0.0f);
			getComms().send(command);
        }
			...
			const FactionConfig@ config = m_factions[i].m_config;
		{
			...
			command.setIntAttribute("faction_id", i);
			command.setStringAttribute("soldier_group_name", "demon sniper");
			command.setFloatAttribute("spawn_score", 0.0f);		// 0.005 during Halloween event
			getComms().send(command);
		}
	...
	protected void setupOffdutyveteran() {
			...
			const FactionConfig@ config = m_factions[i].m_config;
		{
			...
			command.setIntAttribute("faction_id", i);
			command.setStringAttribute("soldier_group_name", "off duty veteran");
			command.setFloatAttribute("spawn_score", 0.0f);		// 0.004 (was 0.008 on launch) during Summer event, else 0.0
			getComms().send(command);
		}
	...
	protected void setupBanana() {
	    _log("Setting up Banana faction...");
		for (uint i = 1; i < m_factions.size(); ++i) {
			const FactionConfig@ config = m_factions[i].m_config;
		{
			XmlElement command("command");
			command.setStringAttribute("class", "faction_resources");
			command.setStringAttribute("soldier_group_name", "default_ai");
			command.setIntAttribute("faction_id", i);
				XmlElement item("projectile");
				item.setStringAttribute("key", "flare_banana.projectile");
				item.setBoolAttribute("enabled", false);		// item.setBoolAttribute("enabled", true); for Summer event 
	...
	protected void setupPumpkinKnight() {
		{
			// disable Pumpkin Knights in friendly faction
			XmlElement command("command");
			command.setStringAttribute("class", "faction");
			command.setIntAttribute("faction_id", 0);
			command.setStringAttribute("soldier_group_name", "pumpkin knight");
			command.setFloatAttribute("spawn_score", 0.0f);
			getComms().send(command);
        }
	...
	protected void setupElves() {
		_log("Setting up elf faction...");
		for (uint i = 0; i < m_factions.size(); ++i) {
			{
				XmlElement command("command");
				command.setStringAttribute("class", "faction");
				command.setIntAttribute("faction_id", i);
				command.setStringAttribute("soldier_group_name", "elf");
				command.setFloatAttribute("spawn_score", 0.0f);		// 0.008 during xmas event, else 0.0
				getComms().send(command);
			}
		...
		
		{
			...
			command.setStringAttribute("soldier_group_name", "elf");
			command.setIntAttribute("faction_id", 0);
				XmlElement item("carry_item");
				item.setStringAttribute("key", "vest_elf_bomber.carry_item");
				item.setBoolAttribute("enabled", false);    
			command.appendChild(item);
				XmlElement item2("carry_item");
				item2.setStringAttribute("key", "vest_elf_bomber_ally.carry_item");
				item2.setBoolAttribute("enabled", true);
			command.appendChild(item2);
			getComms().send(command);
		}
    }
...
//			CallMarkerConfig("mortar2.call", "call_marker", 7, 0.5, 50.0),            // remplaced in 1.77 with cluster bomb
			...
			CallMarkerConfig("vip.call", "call_marker_drop", 16, 0.5),
			CallMarkerConfig("santa_reward.call", "call_marker_drop", 3, 0.5)
			};
