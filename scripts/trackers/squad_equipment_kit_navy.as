...
class SquadEquipmentKitNavy : Tracker {
	...
	// --------------------------------------------
	SquadEquipmentKitNavy(Metagame@ metagame) {
		@m_metagame = @metagame;
	}

	// --------------------------------------------
	protected void handleItemDropEvent(const XmlElement@ event) {
		string key = "squad_equipment_kit_navy.weapon";
	...
	protected void handleResultEvent(const XmlElement@ event) {
		string key = "squad_equipment_kit_navy";
		...
		if (key == eventKey) {
			...
			// Costumes for faulty kits (mod-present items only); fixed: comma between array elements
