...
| # | Was wird getestet? | Erwartung / Ziel | Vorgehen / Details | Ergebnis | Notizen |
|---|--------------------|------------------|-------------------|----------|--------|
| **1** | **Schatten-Call + call_event** | call_event mit phase `"queue"` und allen nötigen Attributen (character_id, target_position, faction_id, id) erscheint im Log. | Call anlegen (leere round, notify_metagame=1), im Spiel auslösen; Log bzw. Tracker-Ausgabe prüfen. | [OK] | cover_drop.call als Schatten-Call; Log zeigt char, player, xp, pos. |
| **2** | **character_id → player_id** | Aus character_id lassen sich player_id und Spielername zuverlässig ermitteln. | getPlayers() durchlaufen, character_id abgleichen → player_id und name ausgeben (Test-Tracker oder bestehendes Script). | [OK] | Im gleichen Tracker: player_id, name aus getPlayerInfo; für sendPrivateMessage/Fallback nutzbar. |
| **3** | **Sprechblase (position)** | Text mit position erscheint nur für nahe Spieler als Sprechblase; weiter entfernt nicht. | sendPrivateMessage(metagame, playerId, "Test", characterPosition); zweiter Spieler in Reichweite vs. außer Reichweite beobachten. | [ ] | Wenn nein: Faction-Chat (said_as_character) nutzen. |
| **4** | **Faction-Chat „als Soldat“** | Faktion sieht die Meldung als von diesem Soldaten/Charakter gesprochen (Name/Icon stimmen). | sendFactionMessageKeySaidAsCharacter(metagame, factionId, characterId, "key", dictionary(), 1.0); Test-Key in languages/ oder existierenden Key nutzen. | [ ] | z. B. squad_equipment_kit-Key. |
| **5** | **acknowledge_time** | Bewegung ist während der acknowledge-Phase (~5 s) gesperrt. | Call mit acknowledge_time="5.0" anlegen, auslösen, in den ~5 s Bewegung probieren. | [ ] | Nur relevant bei gewünschter Bewegungssperre. |
| **6** | **create_instance Einschlag** | An der Zielposition erfolgt ein Artillerie-Einschlag (Explosion/Schaden). | create_instance (instance_class='grenade', instance_key='artillery_shell.projectile', position=target, faction_id, character_id) aus Test-Tracker bei "queue" senden. | [ ] | ggf. heavy_artillery_shell.projectile vergleichen. |
| **7** | **Vollablauf 5-Sekunden-Sequenz** | Nachrichten exakt bei 0 / 1.5 / 3 / 5 s, Einschlag bei 5 s; Reihenfolge und Texte korrekt. | Fire-Mission-Tracker: bei "queue" Timer starten; bei 0 / 1.5 / 3 / 5 s Nachrichten (Spieler + Commander), bei 5 s create_instance. | [ ] | Zuerst mit festen Texten. |
| **8** | **Abbruch bei Tod + Refund** | Kein Einschlag; RP werden zurückerstattet (rp_reward); Meldung zu Abbruch/Refund sichtbar. | Während laufender Sequenz (vor 5 s) Anforderer töten; handleCharacterDieEvent nur eigene Sequenz abbrechen. | [ ] | |
| **9** | **Zwei Calls gleichzeitig** | Beide Sequenzen laufen fehlerfrei parallel; zwei Einschläge an den jeweiligen Zielen. | Zwei Spieler starten kurz nacheinander den Fire-Mission-Call; Queue/Array pro call_id im Tracker. | [ ] | |
| **10** | **Rang + Name in Meldung** | Erste Spieler-Zeile zeigt „Rang Nachname“ (z. B. „Sergeant Müller“); Rang aus XP, Name aus Soldat oder Fallback. | Rang aus Character-XP + Faction-Tabelle; Name = Character-Attribut oder Fallback (letztes Wort Player-Name / „Soldier“). | [OK] | Log: „Captain Brandt“ (Rang = engl. Key aus Tabelle, in-game DE = Hauptmann; Name aus Character). |
